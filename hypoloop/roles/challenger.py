...
def build_prompt(task: str, root: Path, cfg: Dict[str, Any],
    ...
    blocks = [
        ...
        "   - **corrections：凡是你算出了具体的正确值，必须在这里落成一条。**"
        "写 id / what / wrong / correct / basis 五个字段。这一条是实打实的教训："
        "曾经有一轮，你的前任在 flaw 里正确指出「假设者把所需位移算成了碰撞体全宽"
        "3.70m，几何错了」，验证者采纳了 revise 这个**裁决**，却把 3.70 原样写进"
        "代码，留下一个必死率 9% 的窟窿，下一轮才被抓出来。"
        "**订正只写在自由文本里，就是会被漏掉。** 没有具体数值要订正就交空数组。",
